...
# Define look_back early in the script
look_back = 60  # Number of past days to consider for each prediction
...
def build_and_train_model(params, X_train, y_train):
    log_and_print(f"Building LSTM model with params: {params}")
    model = Sequential([
...
if __name__ == "__main__":
    ...
    # ------------------ SCALE FEATURES ------------------
    # Split data into training and testing to prevent data leakage
    split_ratio = 0.8
    train_size = int(len(data) * split_ratio)
    # Ensure 'look_back' is defined before its use
    # look_back = 60  # Already defined above

    train_data = data.iloc[:train_size]
    test_data = data.iloc[train_size - look_back:]  # Include look_back for creating test sequences
    ...
    def build_and_train_model_optuna(params, X_train, y_train):
        return build_and_train_model(params, X_train, y_train)
    ...
    def objective(trial):
        ...
        model, history = build_and_train_model_optuna(params, X_train, y_train)
        ...
        # Adjusted to include additional features by adding zero columns
        scaled_predictions_extended = np.hstack(
            (predictions, np.zeros((predictions.shape[0], scaled_train.shape[1] - 1))))
        predictions_unscaled = scaler.inverse_transform(scaled_predictions_extended)[:, 0]
        # Similarly adjust y_test
        y_test_extended_val = np.hstack((y_test.reshape(-1, 1), np.zeros((y_test.shape[0], scaled_train.shape[1] - 1))))
    ...
    best_model, best_history = build_and_train_model(best_params, X_train, y_train)
    ...
    # Adjusted to include additional features by adding zero columns
    scaled_predictions_extended = np.hstack((predictions, np.zeros((predictions.shape[0], scaled_train.shape[1] - 1))))
    predictions_unscaled = scaler.inverse_transform(scaled_predictions_extended)[:, 0]
    # Similarly adjust y_test
    y_test_extended_final = np.hstack((y_test.reshape(-1, 1), np.zeros((y_test.shape[0], scaled_train.shape[1] - 1))))
